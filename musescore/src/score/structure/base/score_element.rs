...
pub trait ScoreElement {
	fn sc_el(&self) -> &ScoreElementData;
	fn sc_el_mut(&mut self) -> &mut ScoreElementData;
	...
	fn parent(&self) -> Option<ElementRef> { self.sc_el().parent.as_ref().and_then(ElementRefWeak::upgrade) }
	fn set_parent(&mut self, e: Option<ElementRefWeak>) { self.sc_el_mut().parent = e; }
	...
	fn get_ref(&self) -> ElementRefWeak { self.sc_el().self_ref.clone().expect("Self Ref is not set or invalid!") }
	fn set_ref(&mut self, v: ElementRefWeak) { self.sc_el_mut().self_ref = Some(v) }
}